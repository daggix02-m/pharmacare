...
export const useSignupStore = create((set, get) => ({
  ...
  validateCurrentStep: (step, shouldSetErrors = true) => {
    ...
    if (step === 1) {
      ...
      if (!branchInfo.branchName) newErrors.branchName = 'Branch name is required';
      if (!branchInfo.branchLocation) newErrors.branchLocation = 'Branch location is required';
